...
if($action == 'update'){
    ...
    $arrs = $mapModel->getMap($id);
//    print_r($arrs);
}
if(isset($_REQUEST['add'])){//add
    $mapController = new mapController($_REQUEST, $_FILES);
    if($mapController->addMap() > 0){
        $msg = '添加成功';
    }else{
        $msg = '添加失败';
    }
...
<div class='alert alert-info' id="alert" style="display:none;">
				...
				<form name="ibodyform" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
					<table width="100%" cellpadding="1" cellspacing="1">
						<tr>
							...
							<td height="30"><input type="text" name="name" value="<?php echo @$arrs[MapModel::_name];?>" style="width:250px;margin:0"/></td>
						</tr>
                        <tr>
							<td height="30" width="45%" align="right" style="padding-right:10px;"><?php echo $title;?>备注：</td>
							<td height="30"><input type="text" name="remark" value="<?php echo @$arrs[MapModel::_remark];?>" style="width:250px;margin:0"/></td>
						</tr>
						<tr>
	...
	$mapModel = new MapModel();
    $count = $mapModel->getCounts($_REQUEST[$key]);
	$page = new page($count, $_REQUEST, $param);

    $mapModel->setPage($page);
    $result = $mapModel->getList();
	...
	<table width="100%" cellpadding="1" cellspacing="1" style="border:1px solid #CCCCCC;text-align:center">
		...
		?>
            <tr bgcolor="<?php echo $page->getColor();?>">
                <td height="25"><?php echo $arr[MapModel::_id]; ?></td>
                <td><?php echo $arr[MapModel::_name]; ?></td>
                <td><?php echo $arr[MapModel::_remark]; ?></td>
                <td>
                    <a href="<?php echo $_SERVER['PHP_SELF'];?>?action=update&id=<?php echo $arr[MapModel::_id] . $page->getParams();?>">修改</a>
                </td>
            </tr>
		<?php 
            }
