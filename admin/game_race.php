...
if($action == 'update'){
    ...
    $arrs = $raceModel->getRace($id);
//    print_r($arrs);
}
if(isset($_REQUEST['add'])){//add
    $raceController = new raceController($_REQUEST, $_FILES);
    if($raceController->addRace() > 0){
        $msg = '添加成功';
    }else{
        $msg = '添加失败';
    }
...
if(isset($_REQUEST['update'])){//update
    $raceController = new raceController($_REQUEST, $_FILES);
    if($raceController->updateRace() > 0){
        $msg = '修改成功';
    }else{
        $msg = '修改失败';
    }
...
<div class='alert alert-info' id="alert" style="display:none;">
				...
				<form name="ibodyform" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
					<table width="100%" cellpadding="1" cellspacing="1">
						<tr>
							...
							<td height="30"><input type="text" name="name" value="<?php echo @$arrs[RaceModel::_name];?>" style="width:250px;margin:0"/></td>
						</tr>
                        <tr>
							<td height="30" width="45%" align="right" style="padding-right:10px;"><?php echo $title;?>备注：</td>
							<td height="30"><input type="text" name="remark" value="<?php echo @$arrs[RaceModel::_remark];?>" style="width:250px;margin:0"/></td>
						</tr>
						<tr>
	...
	$raceModel = new RaceModel();
    $count = $raceModel->getCounts($_REQUEST[$key]);
	$page = new page($count, $_REQUEST, $param);

    $raceModel->setPage($page);
    $result = $raceModel->getList();
	...
	<table width="100%" cellpadding="1" cellspacing="1" style="border:1px solid #CCCCCC;text-align:center">
		...
		?>
            <tr bgcolor="<?php echo $page->getColor();?>">
                <td height="25"><?php echo $arr[RaceModel::_id]; ?></td>
                <td><?php echo $arr[RaceModel::_name]; ?></td>
                <td><?php echo $arr[RaceModel::_remark]; ?></td>
                <td>
                    <a href="<?php echo $_SERVER['PHP_SELF'];?>?action=update&id=<?php echo $arr[RaceModel::_id] . $page->getParams();?>">修改</a>
                </td>
            </tr>
		<?php 
            }
