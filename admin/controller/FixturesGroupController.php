 ...
 * @author koudejian
 */
class FixturesGroupController extends BaseController{
    private $file = null;
...
//        exit;
     ...
     * @return boolean
     */
    public function addFixturesGroup(){
        //1.接收参数
        $name = @$this->args['name'];
        ...
        //3.插入数据
        $fixturesGroupMode = new FixturesGroupModel();
        $result = $fixturesGroupMode->add($name, $remark);
    ...
    public function updateFixturesGroup(){
        ...
        //3.修改数据
        $fixturesGroupMode = new FixturesGroupModel();
        $result = $fixturesGroupMode->update($id, $name, $remark);
        if($result == false || $result == null){
            return false;
        }
        return $result;
