...
if($action == 'update'){
    ...
    $arrs = $coachModel->getCoach($id);
//    print_r($arrs);
}
if(isset($_REQUEST['add'])){//add
    $coachController = new CoachController($_REQUEST, $_FILES);
    if($coachController->addCoach() > 0){
        $msg = '添加成功';
    }else{
        $msg = '添加失败';
    }
...
if(isset($_REQUEST['update'])){//update
    $coachController = new CoachController($_REQUEST, $_FILES);
    if($coachController->updateCoach() > 0){
        $msg = '修改成功';
    }else{
        $msg = '修改失败';
    }
...
<div class='alert alert-info' id="alert" style="display:none;">
				...
				<form name="ibodyform" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" enctype="multipart/form-data">
					<table width="100%" cellpadding="1" cellspacing="1">
						<tr>
							...
							<td height="30"><input type="text" name="name" value="<?php echo @$arrs[CoachModel::_name];?>" style="width:250px;margin:0"/></td>
						</tr>
                        <tr>
							<td height="30" width="45%" align="right" style="padding-right:10px;"><?php echo $title;?>备注：</td>
							<td height="30"><input type="text" name="remark" value="<?php echo @$arrs[CoachModel::_remark];?>" style="width:250px;margin:0"/></td>
						</tr>
						<tr>
	...
	$coachModel = new CoachModel();
    $count = $coachModel->getCounts($_REQUEST[$key]);
	$page = new page($count, $_REQUEST, $param);

    $coachModel->setPage($page);
    $result = $coachModel->getList();
	...
	<table width="100%" cellpadding="1" cellspacing="1" style="border:1px solid #CCCCCC;text-align:center">
		...
		?>
            <tr bgcolor="<?php echo $page->getColor();?>">
                <td height="25"><?php echo $arr[CoachModel::_id]; ?></td>
                <td><?php echo $arr[CoachModel::_name]; ?></td>
                <td><?php echo $arr[CoachModel::_remark]; ?></td>
                <td>
                    <a href="<?php echo $_SERVER['PHP_SELF'];?>?action=update&id=<?php echo $arr[CoachModel::_id] . $page->getParams();?>">修改</a>
                </td>
            </tr>
		<?php 
            }
